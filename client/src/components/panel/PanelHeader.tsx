      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-4">
        ...
        <div className="pt-3 border-t border-slate-100 flex items-center gap-3 flex-wrap text-xs">
          ...
          {interviewers.length === 0 ? (
            <span className="text-slate-400 italic">No interviewers assigned yet. Click "Edit Panel & Team" to add.</span>
          ) : (
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  {int.domains?.map((dom: any) => (
                    <span
                      key={dom._id || dom}
                      className="text-[10px] px-1.5 py-0.2 rounded bg-blue-100 text-blue-800 font-mono font-medium"
                    >
                      {dom.name || dom}
                    </span>
                  ))}
