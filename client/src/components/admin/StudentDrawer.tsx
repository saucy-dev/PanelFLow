      ...
      <div className="space-y-6">
        ...
        <div className="bg-slate-50 rounded-xl p-4 border border-slate-200/80 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Candidate Info</span>
              ...
              {student.domainPreferences?.map((pref: any, idx: number) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[10px] uppercase font-bold text-slate-400">Rank {pref.priority}</span>
                  </div>
                );
              })}
            </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
              <Building2 className="w-4 h-4 text-blue-600" /> Available Matching Panels
            </h3>
          ...
          {rankedAvailablePanels.length === 0 ? (
            <div className="p-4 text-center rounded-xl bg-amber-50/50 border border-amber-200 text-xs text-amber-800">
                    ...
                    <div className="space-y-1 min-w-0">
                      ...
                      {isStrong ? (
                        <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-700 bg-emerald-100/80 px-2 py-0.5 rounded">
                          <Sparkles className="w-3 h-3 fill-emerald-600" /> {match.label}
                        </span>
                      ) : isGood ? (
                        <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-blue-700 bg-blue-100/80 px-2 py-0.5 rounded">
                          {match.label}
                        </span>
                      ) : (
                        <span className="text-[11px] text-slate-500 italic">No direct domain match</span>
                      )}
          ...
          <div className="space-y-2 pt-3 border-t border-slate-100">
            ...
            <div className="grid grid-cols-2 gap-2">
              {otherPanels.map((p) => (
                ...
                >
                  <span className="font-mono font-bold text-slate-800">{p.panelCode}</span>
