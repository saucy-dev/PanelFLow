      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <label className="cursor-pointer inline-flex items-center gap-1 text-xs font-semibold text-blue-600 hover:text-blue-700 bg-blue-50 px-2.5 py-1.5 rounded-lg border border-blue-200">
              <Upload className="w-3.5 h-3.5" />
              <span>Upload .csv File</span>
              <input type="file" accept=".csv,.txt" onChange={handleFileUpload} className="hidden" />
            </label>

            <Button size="sm" variant="ghost" onClick={handleLoadSample} className="text-xs h-8">
        ...
        <div>
          <textarea
            ...
            rows={6}
            className="w-full text-xs font-mono p-3 bg-slate-50 border border-slate-300 rounded-xl focus:bg-white focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
          />
        </div>

        {/* Validation Preview Results */}
        {previewResult && (
          <div className="border border-slate-200 rounded-xl overflow-hidden bg-white space-y-2">
            <div className="p-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-700">Validation Summary</span>
              ...
              {previewResult.preview.slice(0, 10).map((row, idx) => (
                <div
                  ...
                  className={`p-2.5 flex items-center justify-between gap-2 ${
                    row.isValid ? 'bg-white' : 'bg-rose-50/50 text-rose-900'
                  }`}
                >
                  ...
                  {row.isValid ? (
                    <span className="text-emerald-600 text-[11px] font-semibold shrink-0">Ready</span>
                  ) : (
                    <span className="text-rose-600 text-[11px] font-semibold shrink-0">
                      {row.errors.join(', ')}
                    </span>
                  )}
                </div>
